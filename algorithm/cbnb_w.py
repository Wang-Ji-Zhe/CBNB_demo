...
from algorithm.pcmci_with_bk import PCMCI as PCMCIbk
...
# 在CBNBw类的__init__方法中，增加一个先验知识的矩阵作为输入参数，并保存在self.prior_knowledge属性中
class CBNBw:
    ...
    # 在CBNBw类的constraint_based方法中，调用PCMCI+算法时，传入先验知识的矩阵作为prior_matrix参数
    def constraint_based(self):
        ...
        # 修改代码：传入先验知识selected_links
        output = pcmci.run_pcmciplus(selected_links=self.selected_links, tau_min=0, tau_max=self.tau_max, pc_alpha=self.sig_level, )
